...
function replaceTextNodes(element, translation) {
    ...
    while ((node = walker.nextNode())) {
        // Check if the text node's content matches the key (trimmed)
        if (node.nodeValue.trim() === element.dataset.translateKey) {
            node.nodeValue = translation;
            break; // Exit after first replacement (assumes single key per element)
        }
    }
}
...
function applyTranslations() {
    try {
        ...
        document.querySelectorAll(translationSelectors.text).forEach(element => {
            ...
            if (translations[key]) {
                // Use innerHTML if element has data-translate-html
